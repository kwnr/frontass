from PySide6.QtWidgets import QApplication, QMainWindow, QPushButton, QLineEdit, QDialogButtonBox
...
from interass.msg import  UI2Robot, Robot2UI, Hold, Preset, UIAction, PoseIteration
...
class UI(QMainWindow, Ui_MainWindow):
    def __init__(self, node: Node):
        ...
        node.create_subscription(Robot2UI, "robot_to_ui", self.cb_subscribe_robot_state, 10)
        self.hold_pub = node.create_publisher(Hold, "hold", 10)
        self.ui_action_pub = node.create_publisher(UIAction, "ui_action", 10)
        ...
        # ui button actions
        self.power_btn.toggled.connect(lambda: self.value_toggle(self.power_btn, self.robot_power, 0))
        self.power_btn.toggled.connect(self.cb_ui_action)

        # pump_frame
        self.pump_power_btn.toggled.connect(lambda: self.value_toggle(self.pump_power_btn, self.pump_power, 0))
        self.pump_mode_btn.toggled.connect(lambda: self.value_toggle(self.pump_mode_btn, self.pump_mode, 0))
        ...
        self.preset_pub = node.create_publisher(Preset, "preset", 10)
        self.preset_mode_btn.clicked.connect(self.preset_diag.show)

        self.preset_diag.joint_spinbox_list[0].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[1].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[2].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[3].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[4].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[5].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[6].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[7].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[8].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[9].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[10].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[11].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[12].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[13].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[14].valueChanged.connect(self.get_preset_spinbox_val)
        self.preset_diag.joint_spinbox_list[15].valueChanged.connect(self.get_preset_spinbox_val)
        ...
        self.preset_diag.buttonBox.clicked.connect(
            lambda btn: self.cb_req_preset_srv()
            if self.preset_diag.buttonBox.standardButton(btn) == QDialogButtonBox.Apply
        ...
        self.l1_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l1_power_btn, self.is_on_hold, 0, self.hold_pub))
        self.l2_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l2_power_btn, self.is_on_hold, 1, self.hold_pub))
        self.l3_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l3_power_btn, self.is_on_hold, 2, self.hold_pub))
        self.l4_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l4_power_btn, self.is_on_hold, 3, self.hold_pub))
        self.l5_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l5_power_btn, self.is_on_hold, 4, self.hold_pub))
        self.l6_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l6_power_btn, self.is_on_hold, 5, self.hold_pub))
        self.l7_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l7_power_btn, self.is_on_hold, 6, self.hold_pub))
        self.l8_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.l8_power_btn, self.is_on_hold, 7, self.hold_pub))

        self.r1_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r1_power_btn, self.is_on_hold, 8, self.hold_pub))
        self.r2_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r2_power_btn, self.is_on_hold, 9, self.hold_pub))
        self.r3_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r3_power_btn, self.is_on_hold, 10, self.hold_pub))
        self.r4_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r4_power_btn, self.is_on_hold, 11, self.hold_pub))
        self.r5_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r5_power_btn, self.is_on_hold, 12, self.hold_pub))
        self.r6_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r6_power_btn, self.is_on_hold, 13, self.hold_pub))
        self.r7_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r7_power_btn, self.is_on_hold, 14, self.hold_pub))
        self.r8_power_btn.toggled.connect(
            lambda: self.pub_hold_toggle(self.r8_power_btn, self.is_on_hold, 15, self.hold_pub))
        ...
        self.pose_iter_publisher = node.create_publisher(PoseIteration, "pose_iter", 5)
        ...
        self.start_time = node.get_clock().now().seconds_nanoseconds()[0]
        # graph - arm
        self.arm_canvas = FigureCanvas(Figure(figsize=(6.4, 7)))
        self.arm_ax, self.pump_ax = self.arm_canvas.figure.subplots(nrows=2, ncols=1, sharex=True)
        self.pos_lines: List[Line2D] \
            = self.arm_ax.plot(0, np.zeros((1, 16)),
                               linestyle="solid", linewidth=2,
                               label=[f"L{i} pos" for i in range(1, 9)] + [f"R{i} pos" for i in range(1, 9)],
                               animated=True)
        self.ref_lines: List[Line2D] \
            = self.arm_ax.plot(0, np.zeros((1, 16)),
                               linestyle="dashed", linewidth=2,
                               label=[f"L{i} ref" for i in range(1, 9)] + [f"R{i} ref" for i in range(1, 9)],
                               animated=True)
        self.cmd_lines: List[Line2D]\
            = self.arm_ax.plot(0, np.zeros((1, 16)),
                               linestyle="dotted", linewidth=2,
                               label=[f"L{i} cmd" for i in range(1, 9)] + [f"R{i} cmd" for i in range(1, 9)],
                               animated=True)
        self.err_lines: List[Line2D]\
            = self.arm_ax.plot(0, np.zeros((1, 16)),
                               linestyle=(0, (5, 1)), linewidth=2,
                               label=[f"L{i} err" for i in range(1, 9)] + [f"R{i} err" for i in range(1, 9)],
                               animated=True)
        self.err_i_lines: List[Line2D]\
            = self.arm_ax.plot(0, np.zeros((1, 16)),
                               linestyle=(0, (3, 1, 1, 1)), linewidth=2,
                               label=[f"L{i} err_i" for i in range(1, 9)] + [f"R{i} err_i" for i in range(1, 9)],
                               animated=True)
        self.err_d_lines: List[Line2D]\
            = self.arm_ax.plot(0, np.zeros((1, 16)),
                               linestyle=(0, (3, 1, 1, 1, 1, 1, 1)), linewidth=2,
                               label=[f"L{i} err_d" for i in range(1, 9)] + [f"R{i} err_d" for i in range(1, 9)],
                               animated=True)       
        ...
        self.pump_etc_ax.legend(handles=[self.pump_rpm_lines[0], self.pump_rpm_lines[1],
                                         self.pump_cur_line, self.pump_elmo_temp_line],
                                loc="upper left")
        self.pump_ax.set_autoscaley_on(True)
        self.pump_etc_ax.set_autoscaley_on(True)
        self.pump_ax.grid(True)

        self.bm = BlitManager(self.arm_canvas, [self.arm_ax, self.pump_ax, self.pump_etc_ax,
                                                *self.pos_lines, *self.ref_lines, *self.cmd_lines,
                                                *self.err_lines, *self.err_i_lines, *self.err_d_lines,
                                                *self.pump_rpm_lines, self.pump_cur_line, self.pump_elmo_temp_line,
                                                ])
    ...
    def cb_ui_action(self):
        ...
        req = UIAction(pump_pwr=pump_pwr,
                        pump_mode=pump_mode,
    ...
    def get_preset_spinbox_val(self):
        ...
        if self.preset_diag.apply_inst_cbx.isChecked():
            res = self.preset_pub.publish(self.preset_diag.enable_preset_mode_btn.isChecked(), self.preset_pos.tolist())
            print(res)
    ...
    def update_figure(self):
        ...
        self.arm_ax.legend(handles=list(compress(self.pos_lines, self.fig_sel_diag.checkbox_state[0]))
    ...
    def cb_subscribe_robot_state(self, data: Robot2UI):
        ...
        self.time_sec = data.header.stamp.to_sec()
    ...
    def disp_track_progress_bar(self, track_left_raw, track_right_raw):
        ...
        left_forward_value, left_backward_value = self.map_value_as_percent(track_left_raw, 0, 1000)
        right_forward_value, right_backward_value = self.map_value_as_percent(track_right_raw, 0, 1000)
        ...
        self.tr_right_status_text.setText(f"{right_forward_value-right_backward_value}%")

    @staticmethod
    def map_value_as_percent(raw_value: int, min_value: int, max_value: int):
        value = (raw_value - min_value) * (100 - (-100)) // (max_value - min_value) + (-100)
        if value >= 0:
            pos_val = value
            neg_val = 0
        else:
