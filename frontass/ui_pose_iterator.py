...
from PySide6.QtWidgets import QDialog, QToolButton, QLineEdit, QFileDialog, QTableWidget, QTableWidgetItem
...
class UIPoseIterator(QDialog, Ui_Dialog):
    def __init__(self, publisher):
        ...
        self.conv_crit_line.editingFinished.connect(lambda: self.set_converge_criterion(self.conv_crit_line.text()))
    ...
    def open_csv_file(self):
        self.file_path, _ = QFileDialog.getOpenFileName(self, caption="Select Pose File",
        ...
        if self.file_path != "":
            ...
            self.iter_table.setHorizontalHeaderLabels(["Now", *self.pose_df.columns.astype(str)])

            for col in range(self.pose_df.shape[1]):
                for row, pose in enumerate(self.pose_df.iloc[:, col]):
                    self.iter_table.setItem(row, col+1, QTableWidgetItem(f"{pose:.2f}"))
    ...
    def update_pos_col(self, pos: np.ndarray, converged: np.ndarray):
        for i, p in enumerate(pos):
            ...
            item.setBackground(QColor(0x00, 0xcd, 0x00) if converged[i] else QColor(0xff, 0xff, 0xff))
        ...
        item = QTableWidgetItem(f"{time.time()-self.pos_start_time:.2f}" if self.pos_start_time != -1 else "")
        self.iter_table.setItem(18, 0, item)

    def determine_converged(self, pos: np.ndarray, tgt_pose: np.ndarray) -> np.ndarray:
        if tgt_pose.size != 0:
            converged = np.less_equal(abs(tgt_pose - pos), self.converge_criterion,
                                      out=np.array([True]*16, dtype=bool), where=~np.isnan(tgt_pose))
            return converged
        else:
    ...
    def timer_loop(self, pos: np.ndarray):
        ...
        if self.enabled_btn.isChecked():
            if not self.is_traj_completed and not self.on_traj_exec:
                ...
                self.publisher.publish(self.enabled_btn.isChecked(), self.trajectory[self.traj_idx], self.trigger)
            elif not self.is_traj_completed:
                if self.traj_idx < len(self.trajectory):
                    self.publisher.publish(self.enabled_btn.isChecked(), self.trajectory[self.traj_idx], self.trigger)
                    self.traj_idx = int((time.time() - self.time_started)/self.exec_time * len(self.trajectory))
                else:
                    self.publisher.publish(self.enabled_btn.isChecked(), self.trajectory[-1], self.trigger)
                    self.is_traj_completed = True
                    self.on_traj_exec = False
            elif converged.all() and not self.on_wait:
                self.on_wait = True
                t_wait = self.pose_df.loc["wait_time"][0]*1000 if ~np.isnan(self.pose_df.loc["wait_time"][0]) else 0
    ...
    def publish(self):
        ...
        print(self.trigger)
        self.publisher.publish(enabled=enabled, poses=self.tgt_pose, trigger=self.trigger)
